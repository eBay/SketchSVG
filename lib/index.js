...
const generateHtmlPage = (resolvedObj) => {
    fs.readFile(`${__dirname}/index.html`).then((data) => {
        const $ = cheerio.load(data.toString());
        $('#svg_demo, #svg_area, #base64_area').empty();
        ...
        fs.writeFile(`${__dirname}/index.html`, $.html()).then(() => {
            console.log(colors.red('Need to make customizations? Use our fancy editor for that!'
                , `${__dirname}/index.html`));
        });
    });
};

const runCmdLine = (allSlices, fileName) => new Promise((resolve, reject) => {
...
const getSVGs = async(filePath, items) => new Promise(async(resolve, reject) => {
    const arrayOfSVGs = [];
    for (let i = 0; i < items.length; i++) {
        const file = `${filePath}/${items[i]}`;
        if (file.split('.').pop() === 'svg') {
            await util(`${filePath}/${items[i]}`).then(val => {
...
const finalizeHtmlAndCleanup = () => new Promise(async(resolve, reject) => {
    await fs.remove(`${__dirname}/tmpsvgs`);
    await fs.remove(`${__dirname}/temp.sketch`);
    console.log(colors.cyan(`One sec! Sending temp files to the 
                    ${emoji.emojify(':recycle: ')} Bin ...`));
    console.log('\nAll Done! Have a great day!\n');
    resolve();
});

parseSketch(filename).then((value) => {
    let allLayers = shell.exec(`${sketchTool} list layers ${value}`, { silent: true });
    allLayers = JSON.parse(allLayers).pages[0].layers[0];
    runCmdLine(allLayers, value).then(() => {
        ...
        fs.readdir(filePath, (err, items) => {
            if (err) throw err;
            getSVGs(filePath, items).then(async(arrValue) => {
                generateHtmlPage(arrValue);
                await finalizeHtmlAndCleanup();
            });
        });
    });
});
