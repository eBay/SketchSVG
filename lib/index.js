...
const SVGSketch = module.exports = class SVGSketch {
    constructor() {
    }

    init() {
        new ParseSketch().init(filename).then((value) => {
            let allLayers = shell.exec(`${sketchTool} list layers ${value}`, { silent: true });
            allLayers = JSON.parse(allLayers).pages[0].layers[0];
            this.runCmdLine(allLayers, value).then(() => {
                ...
                fs.readdir(filePath, (err, items) => {
                    if (err) throw err;
                    this.getSVGs(filePath, items).then(async(arrValue) => {
                        this.generateHtmlPage(arrValue);
