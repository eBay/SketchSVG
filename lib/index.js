...
// let test = JSON.parse(shell.exec(`${sketchTool} list layers ${filename}`, {silent:false}));
...
parseSketch(filename).then((value)=> {
    ...
    runCmdLine(allLayers, value).then(() => {
        console.log('FILES CONVERTED CT: '+ count);
...
const generateHtmlPage = (resolvedObj) => {
    // const svgs = resolvedObj.svg;
    // const base64s = resolvedObj.base64;
    // const finalName = resolvedObj.name;
    fs.readFile(__dirname + '/index.html').then((data) => {
        ...
        resolvedObj.forEach((val, idx) => {
            ...
            const setSVGTemplate = (name, idx) => {return `<svg data-name='${name}' class='svg_icn svg_icon_${idx}' focusable="false" height="24" width="24"><use xlink:href="#${name}"></use></svg>`};
