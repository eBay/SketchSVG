    ...
    init(filename) {
        return new Promise((resolve, reject) => {
            ns.read(filename).then((sketch = {}) => {
                ...
                let pageLayers = sketch.pages[0].layers;
                const deDupeNames = [];

                if (pageLayers[0]._class === 'artboard') {
                ...
                pageLayers.forEach((instance, outerIdx) => {
                    ...
                    if (instance._class === 'group') {
                        ...
                        this.loopThroughGroup(instance.symbolMaster);
                    }
                });

                const bigGroup = this.createGroup('sketchSVGTempGroup', this.foreignSymbolsArr);
